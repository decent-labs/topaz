...
func TestNew(t *testing.T) {
	...
	token := NewResetToken(testLogin, 100*time.Second, pwdVal, testSecret)
	login, err := VerifyResetToken(token, getPwdVal, testSecret)
	if err != nil {
...
func TestVerify(t *testing.T) {
	...
	// Test expiration
	pwdVal, _ := getPwdVal(testLogin)
	token := NewResetToken(testLogin, -1, pwdVal, testSecret)
	if _, err := VerifyResetToken(token, getPwdVal, testSecret); err == nil {
	...
	token = NewResetToken(login, 100*time.Second, testPwdVal, testSecret)
	if _, err := VerifyResetToken(token, getPwdVal, testSecret); err != errVal {
